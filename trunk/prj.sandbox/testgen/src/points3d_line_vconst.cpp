...
const double l = 15;//расстояние между фарами
...
void Track::print_motion( cv::Mat1b& img, ofstream& out )
 {
   std::vector< cv::Point > img_points(m_points.size());//с ошибкой

   //std::vector< cv::Point3d > img_points(m_points.size());//!! точно восстанавливает потом
   for(size_t i = 0; i < m_points.size(); i++)
   {
     //img_points[i].x = -1 * (F * m_points[i].x / m_points[i].z) + 256;//вынесено в отрисовку
     //img_points[i].y = F * m_points[i].y / m_points[i].z + 512;

     img_points[i].x = round( F * m_points[i].x / m_points[i].z );//координаты в 3d относительно камеры, в фокальной плоскости
     img_points[i].y = round( F * m_points[i].y / m_points[i].z );
     double right = F * (m_points[i].y + l) / m_points[i].z;//левая точка фары


     out << img_points[i].x << " " << img_points[i].y << " "
       << m_points[i].x << " " << m_points[i].y << " " << m_points[i].z << " " /* i <<*/ //endl;
       << right << " " << m_points[i].y + l << endl; // на фокальной плоскости /в мире смещение вправо на l
   }
...
//Вывод в файл:
//1с) # прямых_ количество кадров (т.е. точек на каждой прямой)
...
{  
  ...
  for( size_t i = 0; i < max_img_count; i++ )//по количеству рисунков
  {
    ...
    out << i+1 << " " << frame_count << " " << l << endl;//количество прямых -> в файл / количество кадров = точек / расстояние между фарами 
